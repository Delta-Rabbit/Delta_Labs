...
export function VideoPlayerUI({ className = "", videoSrc }: VideoPlayerUIProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setInternalVideo(url)
    }
  }

  const displayVideo = videoSrc || internalVideo
    ...
    <div className={`relative w-[60vw] h-[258.5px] ${className}`}>
      ...
      {displayVideo ? (
        <video
          src={displayVideo}
          controls
          className="w-full h-full rounded-lg object-cover border-2 border-[#D9D9D9]"
        />
      ) : (
        <>
          ...
          <div className="absolute top-[170px] left-1/2 transform -translate-x-1/2 text-center">
            <p className="text-gray-400 text-sm">You can upload course intro video here</p>
          </div>

          <div
            className="absolute bottom-[20px] left-1/2 transform -translate-x-1/2 w-fit h-[46px] bg-[#174A5F] rounded flex items-center px-4 gap-3 cursor-pointer"
