...
function RejectedTable({ searchQuery = "" }: RejectedTableProps) {
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = "rejected_schools.json"
                ...
                {filteredData.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b border-gray-200">
                      ...
                      <td className="p-3 bg-[#b4e1b0] text-sm text-gray-700 min-w-[200px] h-[57px]">{row.amendment}</td>
