...
const CourseLayout: React.FC<CourseLayoutProps> = ({ children, onSwitchToDemo, onNavigateToOfflineCourses, onNavigateToUnrolledCourses, onNavigateToRecentActivity, onNavigateToSuperCourse }) => {
  ...
  return (
    <>
      ...
      <div className={`bg-white relative z-20 sticky top-[60px] transition-all duration-300 ease-in-out ${
        showSecondaryNav 
        ...
        <div className="container mx-auto px-4 md:px-8 py-3 md:py-4">
          <div className="flex items-center justify-center overflow-x-auto">
            ...
            <div className="flex items-center space-x-6 md:space-x-8 flex-shrink-0">
              {/* Super Course */}
              <button
                onClick={onNavigateToSuperCourse || (() => setCurrentView('superCourse'))}
                className={`flex items-center space-x-2 transition-all duration-300 whitespace-nowrap group cursor-pointer px-4 py-3 relative ${getActiveStyles('superCourse').button}`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${getActiveStyles('superCourse').iconBg}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                </div>
                <span className={`text-sm hidden sm:inline transition-colors duration-300 ${getActiveStyles('superCourse').text}`}>Super Course</span>
              </button>

              {/* Recent Activity */}
              <button
                onClick={onNavigateToRecentActivity || (() => {})}
                className={`flex items-center space-x-2 transition-all duration-300 whitespace-nowrap group cursor-pointer px-4 py-3 relative ${getActiveStyles('recentActivity').button}`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${getActiveStyles('recentActivity').iconBg}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <span className={`text-sm hidden sm:inline transition-colors duration-300 ${getActiveStyles('recentActivity').text}`}>Recent Activity</span>
              </button>
              ...
              <button
                ...
                className={`flex items-center space-x-2 transition-all duration-300 whitespace-nowrap group cursor-pointer px-4 py-3 relative ${getActiveStyles('offlineCourses').button}`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${getActiveStyles('offlineCourses').iconBg}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                  </svg>
                </div>
                <span className={`text-sm hidden sm:inline transition-colors duration-300 ${getActiveStyles('offlineCourses').text}`}>Offline Courses</span>
              </button>

              {/* Message */}
              <button className={`flex items-center space-x-2 transition-all duration-300 whitespace-nowrap group px-4 py-3 relative ${getActiveStyles('message').button}`}>
              ...
              {/* Unrolled Courses */}
              <button
                onClick={onNavigateToUnrolledCourses || (() => {})}
                className={`flex items-center space-x-2 transition-all duration-300 whitespace-nowrap group cursor-pointer px-4 py-3 relative ${getActiveStyles('unrolledCourses').button}`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${getActiveStyles('unrolledCourses').iconBg}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                </div>
                <span className={`text-sm hidden sm:inline transition-colors duration-300 ${getActiveStyles('unrolledCourses').text}`}>Unrolled Courses</span>
              </button>
            </div>
